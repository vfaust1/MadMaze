...
public class ParameterController {
    ...
    public ParameterController(ParameterView view) {
        this.view = view;
    ...
    public void setupListeners() {
        ...
        view.getConfirmButton().setOnAction(e -> onConfirm(
                view.getSelectedMazeType(), 
                view.getWidthValue(),
                view.getHeightValue(),
                view.getMazeParamValue()    // pourcentage ou distance mini selon le type
        ));

        view.getPerfectLabButton().setOnAction(e -> onMazeTypeSelected(MazeType.PERFECT));
        view.getRandomLabButton().setOnAction(e -> onMazeTypeSelected(MazeType.RANDOM));
        ...
        setupSpinnerControls(view.getParameterSpinner(), 5, 300);
    }
    ...
    private void onMazeTypeSelected(MazeType type) {
        view.setSelectedMazeType(type);
        
        if (type == MazeType.PERFECT) {
            view.getParameterLabel().setText("Distance minimale");
            view.getParameterSpinner().setValueFactory(
                new SpinnerValueFactory.IntegerSpinnerValueFactory(10, 300, 20)
            );
            setupSpinnerControls(view.getParameterSpinner(), 10, 300);
        } else {
            view.getParameterLabel().setText("Pourcentage de murs");
            view.getParameterSpinner().setValueFactory(
                new SpinnerValueFactory.IntegerSpinnerValueFactory(10, 80, 35)
            );
            setupSpinnerControls(view.getParameterSpinner(), 10, 80);
        }
    }
    ...
    /*private void onPerfectSelected() {
        view.setSelectedMazeType(MazeType.PERFECT);
    }

    private void onRandomSelected() {
        view.setSelectedMazeType(MazeType.RANDOM);
    }*/
}
